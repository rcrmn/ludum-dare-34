...
namespace UnityStandardAssets.Cameras
{
	...
	public class AutoCam : PivotBasedCameraRig
	{
		...
		[Range(0f, 3f)] [SerializeField] private float m_CursorSpeed = 1.5f;   // How fast the rig will rotate from user input.
		...
		private Vector2 m_MousePivot = new Vector2();
		[SerializeField] private Vector2 m_MaxMousePivot = new Vector2(45, 30);
		...
		void Update()
		{
			...
			m_MousePivot.x += x * m_CursorSpeed;
			m_MousePivot.y += y * m_CursorSpeed;
			if(Math.Abs(m_MousePivot.x) > m_MaxMousePivot.x)
			{
				m_MousePivot.x = Math.Sign(m_MousePivot.x) * m_MaxMousePivot.x;
			}
			if(Math.Abs(m_MousePivot.y) > m_MaxMousePivot.y)
			{
				m_MousePivot.y = Math.Sign(m_MousePivot.y) * m_MaxMousePivot.y;
			}
		}
		...
		protected override void FollowTarget(float deltaTime)
		{
			...
			transform.rotation *= Quaternion.AngleAxis(m_MousePivot.x, Vector3.up);
			transform.rotation *= Quaternion.AngleAxis(m_MousePivot.y, -Vector3.right);
		}
	}
}
